...
#+begin_src json
{
  ...
  "version": "1.0.0",
  "description": "Portfolio222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222",
  "main": "index.html",
  "scripts": {
...
**
** DONE Learning Angular as a Java Developer :angular:typescript:guide:blog:
CLOSED: [2023-03-22 Fri 23:03]
:properties:
:hugo_section: posts/2023/03
:export_file_name: Learning Angular as a Java Developer.md
:end:
...
** DRAFT Today I learnt : Kubectl commands for debugging :til:kubectl:debugging:

CLOSED: [2023-03-03 Fri 03:
:properties:
:hugo_section: posts/2023/03
:export_file_name: Today I learnt Kubectl commands for debugging.md
:end:
...
*** To get the bin bash inside a pod
#+begin_src kubectl
...
*** Scale up and downscale your pod
#+begin_src kubectl
...
** DONE When to ask for help as a Software Engineer? :softwareengineer:help:watercooler:learning:
CLOSED: [2023-03-14 Tue 14]
:properties:
:hugo_section: posts/2023/03
:export_file_name: When to ask for help as a software engineer.md
:end:
...
** DONE Beware of the side bug :softwareengineer:sidebug:watercooler:learning:
CLOSED: [2023-05-09 Tue ]
:properties:
:hugo_section: posts/2023/05
:export_file_name: Beware of the side bug.md
:end:
...
One thing I find myself doing these days  is focusing on the prioritised issue at hand first.Ask a senior or engineer who has more context and information about the issue at hand.
 And simultaneously note down the topics which I didn't understand completely in another place (sublime or self slack) for me to refer later and learn once the work storm calms down.

This is one way I could to get back to learning new things.....
** DONE Today I learnt : How to check if a url is on 2 way SSL or not ? :til:ssl:2wayssl:openssl:curl:
CLOSED: [2023-06-18 Sun ]
:properties:
:hugo_section: posts/2023/06
:export_file_name: Today I learnt : How to check if a url is on 2 way SSL or not.md
:end:
...
When a client (browser) hits the url and it gives success for example Google.com.It is usually on 1 way ssl.
In this case, only the client authenticates the server.
...
#+begin_src 
❯ curl -v https://exampleOf1wayySSL.com  
...
In this example as you can see only the client asked for the certificate with line =TLSv1.2 (IN), TLS handshake, Certificate (11):=
...
When one makes a CURL call to API/Endpoint (curl -v -k <baseURL/endpoint>) without passing flags such as =--key= and =--cert=, the curl call should fail with a handshake failure.
From the response, make a note of the line with =Request CERT= (example below)
...
*_The server would ask for a client certificate in this case_*
...
Similar to =/etc/ssl/cert.pem=, NSS(Network Security Services) as a trusted CA certificate location. Another example of trusted CA cert location is jdk cacerts for java applications.

All three =/etc/ssl/cert.pem=, =NSS=, and =JDK cacerts= are used to store trusted root certificates but differ in their implementation and usage.
...
Make note of =curl: (35) NSS: client certificate not found (nickname not specified)= where we are not passing any client cert or key and get the expected error.
...
When trying to verify via openssl command, =openssl s_client -servername someExampleDomain -connect someExampleDomain.com:443= ,for a happy case look for =Acceptable client certificate CA names=
...
If you don't see your CA(client's CA) mentioned in the above =Acceptable client certificate CA names= section then that means the URL is not using the client key for authentication and there might have been misconfiguration.
...
One can also verify the same in Java by checking your application logs and enabling a flag =-Djavax.net.debug=ssl= or in some cases =-Djavax.net.debug=all= which gives more verbose logs.
...
These were some of the tricks I used to verify.In future blog posts, I will include how to test using Postman too.
Happy Learning!
